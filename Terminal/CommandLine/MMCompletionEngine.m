...
- (NSMutableArray *)completionsForPartial:(NSString *)partial inDirectory:(NSString *)path;
{
  ...
  if (partial.length == 0) {
    ...
    return [MMUtilities filesAndFoldersInDirectory:path includeHiddenFiles:NO];
  }
  ...
  NSArray *files = [MMUtilities filesAndFoldersInDirectory:path includeHiddenFiles:(fileToComplete.length > 0 && [fileToComplete characterAtIndex:0] == '.')];
...
- (void)prepareCompletions;
{
  ...
  NSString *partialPrefix = [MMCommand unescapeArgument:[argument substringToIndex:partialRange.location]][0];
  NSString *partial = [MMCommand unescapeArgument:[argument substringWithRange:partialRange]][0];
